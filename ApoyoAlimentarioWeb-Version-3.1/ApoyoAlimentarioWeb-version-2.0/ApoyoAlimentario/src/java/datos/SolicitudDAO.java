...
public class SolicitudDAO {
  ...
  public String incluirSolicitud(Solicitud solicitud, ArrayList<Integer> llaves,ArrayList<LinkId> links){
       StringBuilder sql = new StringBuilder();
       StringBuilder sql2 = new StringBuilder();
       StringBuilder sql3 = new StringBuilder();
       StringBuilder sql4 = new StringBuilder();
       sql.append("INSERT INTO SOL (k_numeroSolicitud,q_puntaje,f_fechaSolicitud,n_estadoSolicitud,n_explicacionEstado,k_codigoSupervisor,k_codigoEstudiante, k_codigoconvocatoria) VALUES(?,?,?,?,?,?,?,?)");
       ...
       try {//inserta los valores de solicitud
        ...
        int llaveSolicitud = Integer.parseInt(rs.getString(1));
                ...
                java.sql.Date sqlDate = convert( solicitud.getFecha());
                ps.setDate(3, sqlDate);
                ps.setString(4, solicitud.getEstado());               
                ps.setString(5,"na");
                ps.setInt(6, 1);  
                ps.setString(7, solicitud.getCodEstudianteSt());
                ps.setInt(8, solicitud.getNumConvocatoria());
               ...
               for (int i = 0; i < llaves.size(); i++) {
                ...
                for (int i = 0; i < links.size(); i++) {
                    ...
                    if(links.get(i).getId() == 1 || links.get(i).getId() == 2){
                        ps4.setInt(1,21);
                        ps4.setInt(2,llaveSolicitud); 
                        ps4.setInt(3, links.get(i).getId());
                        ps4.setString(4, links.get(i).getLink());
                    
                        ps4.executeUpdate();     
                    }
                    if(links.get(i).getId() == 3){
                        ps4.setInt(1,12);
                        ps4.setInt(2,llaveSolicitud); 
                        ps4.setInt(3, links.get(i).getId());
                        ps4.setString(4, links.get(i).getLink());
                    ...
                    if(links.get(i).getId() == 5){
                        for(int j = 0; j < llaves.size(); j++){
                            if(llaves.get(j) >= 15 && llaves.get(j) <= 20){
                                ps4.setInt(1,llaves.get(j));
                                ps4.setInt(2,llaveSolicitud); 
                                ps4.setInt(3, links.get(i).getId());
                                ps4.setString(4, links.get(i).getLink());
                    ...
                    if(links.get(i).getId() == 9){
                        for(int j = 0; j < llaves.size(); j++){
                            if((llaves.get(j) >= 1 && llaves.get(j) <= 8)){
                                ps4.setInt(1,llaves.get(j));
                                ps4.setInt(2,llaveSolicitud); 
                                ps4.setInt(3, links.get(i).getId());
                                ps4.setString(4, links.get(i).getLink());
